...
const COLORES = {
  ...
  oscuro: '#083344',
  acento: '#064E3B',
  fondo: 'linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%)',
};

// 📚 GRADOS DE BACHILLERATO (Edita aquí para agregar/quitar grados)
...
const TAREAS_INICIALES: Record<string, Tarea[]> = {
  '1er año': [
    { id: 1, titulo: "Ecuaciones lineales", descripcion: "Resolver ecuaciones", fechaEntrega: "2026-04-21", materia: "Matematica", completado: false },
    { id: 2, titulo: "Literatura venezolana", descripcion: "Analisis de obra", fechaEntrega: "2026-04-22", materia: "Castellano", completado: false }
  ],
  '2do año': [
    { id: 1, titulo: "Quimica basica", descripcion: "Tabla periodica", fechaEntrega: "2026-04-23", materia: "Quimica", completado: false },
    { id: 2, titulo: "Geometria", descripcion: "Areas y volumenes", fechaEntrega: "2026-04-24", materia: "Matematica", completado: false }
  ],
  '3er año': [
    { id: 1, titulo: "Quimica organica", descripcion: "Estudio de compuestos", fechaEntrega: "2026-04-23", materia: "Quimica", completado: false },
    { id: 2, titulo: "Historia contemporanea", descripcion: "Siglo XX", fechaEntrega: "2026-04-25", materia: "Historia", completado: false }
  ],
  '4to año': [
    { id: 1, titulo: "Historia de Venezuela", descripcion: "Linea de tiempo", fechaEntrega: "2026-04-24", materia: "Historia", completado: false },
    { id: 2, titulo: "Fisica mecanica", descripcion: "Leyes de Newton", fechaEntrega: "2026-04-26", materia: "Fisica", completado: false }
  ],
  '5to año': [
    { id: 1, titulo: "Calculo diferencial", descripcion: "Ejercicios de limites", fechaEntrega: "2026-04-25", materia: "Matematica", completado: false },
    { id: 2, titulo: "Proyecto de grado", descripcion: "Entrega de anteproyecto", fechaEntrega: "2026-04-30", materia: "Proyecto", completado: false }
  ]
};

const AVISOS_INICIALES: Record<string, any> = {
  '1er año': { titulo: "Bienvenida", mensaje: "Inicio de clases", importante: true },
  '2do año': { titulo: "Proyecto", mensaje: "Entrega de anteproyecto", importante: false },
  '3er año': { titulo: "Examenes", mensaje: "Preparacion para pruebas", importante: true },
  '4to año': { titulo: "Orientacion", mensaje: "Talleres disponibles", importante: false },
  '5to año': { titulo: "Graduacion", mensaje: "Tramites iniciados", importante: true }
};

const PLAN_EVALUACION_INICIAL: Record<string, PlanEvaluacion[]> = {
  '1er año': [
    { id: 1, periodo: "I Lapso", materia: "Matematica", tipoEvaluacion: "Escrita", porcentaje: 30, fecha: "2026-05-15" }
  ],
  '2do año': [
    { id: 1, periodo: "I Lapso", materia: "Quimica", tipoEvaluacion: "Laboratorio", porcentaje: 35, fecha: "2026-05-16" }
  ],
  '3er año': [
    { id: 1, periodo: "I Lapso", materia: "Quimica", tipoEvaluacion: "Teorico", porcentaje: 30, fecha: "2026-05-17" }
  ],
  '4to año': [
    { id: 1, periodo: "I Lapso", materia: "Historia", tipoEvaluacion: "Ensayo", porcentaje: 40, fecha: "2026-05-19" }
  ],
  '5to año': [
    { id: 1, periodo: "I Lapso", materia: "Matematica", tipoEvaluacion: "Escrita", porcentaje: 30, fecha: "2026-05-20" }
  ]
};
...
const ClassroomPage: React.FC = () => {
  ...
  const agregarGuia = () => {
    ...
    setGuias([...guias, { ...nuevaGuia, id: Date.now(), descripcion: nuevaGuia.descripcion || '', nivel: 'Bachillerato', grado: gradoActual }]);
    ...
    <div style={{ fontFamily: "'Montserrat', sans-serif", background: COLORES.fondo, minHeight: '100vh' }}>
      ...
      <nav style={{ background: `linear-gradient(135deg, ${COLORES.oscuro} 0%, ${COLORES.acento} 100%)`, padding: '1rem 0', position: 'sticky', top: 0, zIndex: 1000 }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
            <h2 style={{ color: 'white', margin: 0 }}>U.E Ciudad Cuatricentenaria</h2>
            ...
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
              <select value={gradoActual} onChange={(e) => setGradoActual(e.target.value)} style={{ padding: '0.5rem 1rem', borderRadius: '25px', border: 'none', background: COLORES.principal, color: 'white', fontWeight: 'bold', cursor: 'pointer' }}>
                {GRADOS_BACHILLERATO.map(g => <option key={g}>{g}</option>)}
              </select>
      ...
      <div style={{ textAlign: 'center', marginTop: '1rem' }}>
        <h2 style={{ ...LIQUID_GLASS.title, fontSize: '1.8rem', margin: 0 }}>
          Bachillerato - {gradoActual} - Sección {seccionActual}
        </h2>
      </div>
              ...
              {guias.map(guia => (
                <div key={guia.id} style={{ ...LIQUID_GLASS.container, padding: '1.2rem', position: 'relative' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center', marginTop: '1rem' }}>
                    ...
                    <a href={guia.pdfUrl} download style={{ background: '#28a745', color: 'white', padding: '0.5rem 1rem', borderRadius: '10px', textDecoration: 'none' }}>⬇️ Descargar</a>
                  </div>
                </div>
              ))}
            </div>
      ...
      <footer style={{ background: '#f8f9fa', borderTop: `1px solid ${COLORES.principal}`, marginTop: '2rem', padding: '1.5rem', textAlign: 'center' }}>
        <span style={{ background: `linear-gradient(135deg, ${COLORES.oscuro} 0%, ${COLORES.principal} 100%)`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontWeight: 'bold' }}>
          U.E Ciudad Cuatricentenaria - Bachillerato
        </span>
      </footer>
    </div>
  );
